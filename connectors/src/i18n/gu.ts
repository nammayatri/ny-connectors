import { LanguageStrings } from './types';

export const gu: LanguageStrings = {
  languageName: 'Gujarati',
  nativeLanguageName: 'ગુજરાતી',

  // Welcome & menu
  welcomeMessage: "👋 સ્વાગત છે! હું તમારો રાઈડ બુકિંગ સહાયક છું.\n\nતમે શું કરવા માગો છો?",
  welcomeBack: '👋 ફરી સ્વાગત છે! તમે શું કરવા માગો છો?',
  bookARide: '🚕 રાઈડ બુક કરો',
  trackRide: '📍 રાઈડ ટ્રેક કરો',
  chooseLanguage: '🌐 ભાષા',
  selectLanguage: '🌐 તમારી પસંદગીની ભાષા પસંદ કરો:',
  languageUpdated: (lang: string) => `✅ ભાષા *${lang}* માં બદલાઈ ગઈ.`,
  moreLanguages: '➕ વધુ ભાષાઓ',

  // Auth flow
  needSignIn: '🔐 રાઈડ ટ્રેક કરવા માટે તમારે સાઇન ઇન કરવું પડશે.\n\nશું તમે રાઈડ બુક કરવા માગો છો?',
  enterPhone: 'પહેલી વાર? તમારો 10 અંકનો મોબાઈલ નંબર દાખલ કરો:',
  invalidPhone: 'અમાન્ય ફોન નંબર. માન્ય 10 અંકનો નંબર દાખલ કરો:',
  enterAccessCode: 'તમારો એક્સેસ કોડ દાખલ કરો (Namma Yatri એપ > પ્રોફાઈલ > અમારા વિશે):',
  authSuccess: "તમે તૈયાર છો! તમારે આ ફરીથી કરવાની જરૂર નથી.",
  authSuccessFirstTime: "તમે તૈયાર છો! ચાલો રાઈડ બુક કરીએ.",
  setupFailed: (err: string) => `સેટઅપ નિષ્ફળ: ${err}\nફરીથી પ્રયાસ કરવા "book" મોકલો.`,
  authFailed: (err: string) => `પ્રમાણીકરણ નિષ્ફળ: ${err}\nફરીથી પ્રયાસ કરવા તમારો ફોન નંબર દાખલ કરો:`,
  sharePhone: 'કૃપા કરીને તમારો ફોન નંબર શેર કરવા નીચેનું બટન દબાવો.',
  shareOwnPhone: "કૃપા કરીને તમારો પોતાનો ફોન નંબર શેર કરો, બીજાનો નહીં.",
  sharePhonePrompt: 'તમારો નંબર શેર કરવા બટન દબાવો:',
  couldNotReadPhone: 'તમારો ફોન નંબર વાંચી શકાયો નહીં. કૃપા કરીને ફરીથી પ્રયાસ કરો.',
  allSet: "તમે તૈયાર છો! ચાલો રાઈડ બુક કરીએ.",
  personNotFound: 'લાગે છે તમે નવા છો! ચાલો તમારું એકાઉન્ટ સેટઅપ કરીએ.',
  otpSent: 'તમારા ફોન પર OTP મોકલવામાં આવ્યો છે. કૃપા કરીને OTP દાખલ કરો:',
  enterOtp: 'તમારા ફોન પર મોકલેલ OTP દાખલ કરો:',
  invalidOtp: 'આ માન્ય OTP નથી. કૃપા કરીને 4 અથવા 6 અંકનો કોડ દાખલ કરો:',
  resendOtp: '🔄 OTP ફરીથી મોકલો',
  otpResent: 'OTP ફરીથી મોકલવામાં આવ્યો છે. કૃપા કરીને તમારો ફોન તપાસો:',
  otpResendFailed: (err: string) => `OTP ફરીથી મોકલી શકાયો નહીં: ${err}`,
  otpVerified: 'ફોન સફળતાપૂર્વક ચકાસાયો!',
  otpVerifyFailed: (err: string) => `OTP ચકાસણી નિષ્ફળ: ${err}\nકૃપા કરીને ફરીથી પ્રયાસ કરો:`,
  askName: 'તમારું નામ શું છે? આ ડ્રાઈવરોને તમને ઓળખવામાં મદદ કરે છે.',
  skipName: '⏭️ છોડો',
  nameUpdated: (name: string) => `ધન્યવાદ, ${name}! તમે તૈયાર છો.`,
  nameUpdateFailed: 'નામ અપડેટ થઈ શક્યું નહીં, પરંતુ ચિંતા ન કરો — તમે બુકિંગ ચાલુ રાખી શકો છો.',

  // Origin & destination
  whereToGo: 'તમે ક્યાં જવા માગો છો?',
  whereToGoWithRoutes: 'તમે ક્યાં જવા માગો છો?\n\nઝડપી રૂટ અથવા સ્થાન પસંદ કરો.\nતમે સ્થાનનું નામ પણ ટાઈપ કરી શકો છો:',
  enterPickupAndDrop: '📍 પિકઅપ-ડ્રોપ દાખલ',
  enterPickupPrompt: 'તમારું પિકઅપ સ્થાન દાખલ કરો, અથવા લોકેશન પિન મોકલો:',
  enterDropPrompt: 'હવે તમારું ડ્રોપ સ્થાન દાખલ કરો, અથવા લોકેશન પિન મોકલો:',
  locationPinReceived: (address: string) => `📍 સ્થાન પ્રાપ્ત થયું: ${address}`,
  favouriteLocations: 'અથવા તમારા સાચવેલા સ્થાનોમાંથી પસંદ કરો:',
  addHome: '🏠 ઘર ઉમેરો',
  addWork: '💼 ઓફિસ ઉમેરો',
  enterHomeAddress: 'તમારા ઘરનું સરનામું દાખલ કરો:',
  enterWorkAddress: 'તમારા ઓફિસનું સરનામું દાખલ કરો:',
  selectLocationFor: (tag: string) => `${tag} માટે સ્થાન પસંદ કરો:`,
  locationSaved: (tag: string) => `✅ ${tag} સાચવ્યું! ચાલો રાઈડ બુક કરીએ.`,
  locationSaveFailed: 'સ્થાન સાચવી શકાયું નથી. તમે પિકઅપ અને ડ્રોપ દાખલ કરીને બુક કરી શકો છો.',
  fromHome: '🏠 ઘરેથી',
  fromWork: '💼 ઓફિસથી',
  moreOptions: '➕ વધુ વિકલ્પો',
  pickSavedOrType: 'સાચવેલું સ્થાન પસંદ કરો અથવા સ્થાનનું નામ ટાઈપ કરો:',
  typePickupPlace: 'તમે ક્યાંથી પિકઅપ કરવા માગો છો?\nશોધવા માટે સ્થાનનું નામ ટાઈપ કરો:',
  selectPickup: 'પિકઅપ સ્થાન પસંદ કરો:',
  searchAgain: '🔍 ફરીથી શોધો',
  pickup: (place: string) => `પિકઅપ: ${place}`,
  whereTo: 'ક્યાં જવું છે?\nશોધવા માટે સ્થાનનું નામ ટાઈપ કરો:',
  whereToWithSaved: 'ક્યાં જવું છે?\n\nસ્થાન પસંદ કરો અથવા સ્થાનનું નામ ટાઈપ કરો:',
  typeDropPlace: 'ક્યાં ઉતરવું છે?\nશોધવા માટે સ્થાનનું નામ ટાઈપ કરો:',
  selectDrop: 'ડ્રોપ સ્થાન પસંદ કરો:',
  drop: (place: string) => `ડ્રોપ: ${place}`,
  noPlacesFound: 'કોઈ સ્થાન મળ્યું નથી. અલગ શોધ અજમાવો:',
  invalidChoice: (max: number) => `અમાન્ય પસંદગી. 1-${max} માંથી પસંદ કરો:`,
  couldNotFindLocations: 'તે સ્થાનો શોધી શકાયા નહીં. ફરીથી પ્રયાસ કરવા "book" મોકલો.',
  fromLabel: (tag: string) => `📍 થી: ${tag}`,

  // Ride search & estimates
  searchingRides: 'રાઈડ શોધી રહ્યા છીએ...',
  noRidesAvailable: 'હાલમાં કોઈ રાઈડ ઉપલબ્ધ નથી. ફરીથી પ્રયાસ કરવા "book" મોકલો.',
  noRidesAvailableRetry: '😔 હાલમાં કોઈ રાઈડ ઉપલબ્ધ નથી. તમે શું કરવા માગો છો?',
  activeRideExists: '🚗 તમારી એક સક્રિય રાઈડ પહેલેથી છે. તે પૂર્ણ અથવા રદ ન થાય ત્યાં સુધી નવી બુકિંગ કરી શકાતી નથી.\n',
  basedOnPastRides: '🕐 તમારી પાછલી રાઈડ્સ પર આધારિત:',
  availableRides: 'ઉપલબ્ધ રાઈડ્સ:',
  availableRidesForRoute: 'તમારા રૂટ માટે ઉપલબ્ધ રાઈડ્સ અહીં છે:',

  // Booking
  booking: (tier: string, fare: number) => `${tier} (₹${fare}) બુક કરી રહ્યા છીએ...`,
  stillSearching: (elapsed: number) => `⏳ હજુ પણ નજીકમાં ડ્રાઈવર શોધી રહ્યા છીએ... (${elapsed}s)\n\nરોકવા માટે "cancel" મોકલો.`,
  rideConfirmed: '🎉 રાઈડ કન્ફર્મ થઈ!\n',
  rideBooked: '✅ રાઈડ બુક થઈ!',
  waitingForDriver: 'ડ્રાઈવર સોંપણીની રાહ જોઈ રહ્યા છીએ...',
  trackYourRide: '📲 તમારી રાઈડ ટ્રેક કરો:',
  track: '📲 ટ્રેક:',
  callDriver: '📞 કૉલ કરો',
  cancelRide: '❌ રદ કરો',
  driverLabel: (name: string) => `👤 ડ્રાઈવર: *${name}*`,
  vehicleLabel: (number: string) => `🔢 વાહન: *${number}*`,
  phoneLabel: (phone: string) => `📞 ફોન: *${phone}*`,
  otpLabel: (otp: string) => `🔑 OTP: *${otp}*`,
  driverPhone: (phone: string) => `📞 ડ્રાઈવરનો નંબર: *${phone}*\n\nતમે તેમને સીધો કૉલ કરી શકો છો.`,
  driverDetailsNotAvailable: 'ડ્રાઈવરની વિગતો હજુ ઉપલબ્ધ નથી. કૃપા કરીને થોડીવાર પછી ફરીથી પ્રયાસ કરો.',
  noActiveRide: 'કોઈ સક્રિય રાઈડ મળી નથી.',

  // No driver found
  noDriverFound: (tier: string) => `😔 3 મિનિટ પછી *${tier}* માટે કોઈ ડ્રાઈવર મળ્યો નહીં.\n\nતમે શું કરવા માગો છો?`,
  retrySameVehicle: '🔄 એ જ વાહન ફરીથી અજમાવો',
  tryDifferentVehicle: '🚗 અલગ વાહન અજમાવો',
  mainMenu: '🏠 મુખ્ય મેનૂ',
  retrying: (tier: string) => `🔄 ${tier} ફરીથી પ્રયાસ કરી રહ્યા છીએ...`,
  tierNotAvailable: (tier: string) => `${tier} હાલમાં ઉપલબ્ધ નથી. અહીં ઉપલબ્ધ વિકલ્પો છે:`,
  tryAgain: '🔄 ફરીથી પ્રયાસ કરો',

  // Status
  activeRide: '📍 સક્રિય રાઈડ\n',
  noActiveRides: 'કોઈ સક્રિય રાઈડ મળી નથી. નવી શરૂ કરવા "book" મોકલો.',
  noActiveRidesBook: '🔍 કોઈ સક્રિય રાઈડ મળી નથી.\n\nશું તમે એક બુક કરવા માગો છો?',

  // Cancel
  cancelConfirm: '⚠️ શું તમે ખરેખર તમારી રાઈડ રદ કરવા માગો છો?',
  cancelConfirmWithDriver: (driver: string, vehicle?: string) =>
    `⚠️ *${driver}*${vehicle ? ` (${vehicle})` : ''} સાથેની રાઈડ રદ કરવી છે?`,
  yesCancelIt: '✅ હા, રદ કરો',
  noKeepIt: '🔙 ના, રાખો',
  rideSearchCancelled: 'રાઈડ શોધ રદ થઈ.',
  rideCancelled: 'રાઈડ રદ થઈ. ✅',
  rideCompleted: 'આ રાઈડ પહેલેથી પૂર્ણ થઈ ગઈ છે અને રદ કરી શકાતી નથી.',
  rideAlreadyCancelled: 'આ રાઈડ પહેલેથી રદ થઈ ગઈ છે.',
  rideInProgress: '⚠️ તમારી રાઈડ પહેલેથી ચાલુ છે અને રદ કરી શકાતી નથી.',
  cancelFailed: (err: string) => `રદ કરી શકાયું નહીં: ${err}`,
  cancelled: 'રદ થયું.',
  whatToDo: '\n\nતમે શું કરવા માગો છો?',

  // SOS & safety
  rideNotStarted: '🕐 રાઈડ હજુ શરૂ થઈ નથી — ડ્રાઈવર રસ્તામાં છે.',
  rideInProgressStatus: '🚗 રાઈડ ચાલુ છે.',
  sosButton: '🚨 SOS',
  call112Button: '📞 112 પર કૉલ કરો',
  sosConfirm: '⚠️ શું તમે SOS અલર્ટ મોકલવા માગો છો? આ ઇમરજન્સી કોન્ટેક્ટ્સ અને નમ્મા યાત્રી સેફ્ટી ટીમને જાણ કરશે.',
  yesTriggerSOS: '🚨 હા, SOS મોકલો',
  noGoBack: '🔙 ના, પાછા જાઓ',
  sosTriggered: '🚨 SOS અલર્ટ મોકલાઈ ગયો છે. સુરક્ષિત રહો — મદદ આવી રહી છે.\n\nતમે તાત્કાલિક ઇમરજન્સી સહાય માટે 112 પર પણ કૉલ કરી શકો છો.',
  sosFailed: (err: string) => `SOS મોકલી શકાયો નથી: ${err}\n\nકૃપા કરીને ઇમરજન્સી મદદ માટે સીધો 112 પર કૉલ કરો.`,
  markSafeButton: '✅ સુરક્ષિત ચિહ્નિત કરો',
  markSafeConfirm: 'શું તમે તમારી રાઈડને સુરક્ષિત ચિહ્નિત કરવા માગો છો? આ SOS અલર્ટ રદ કરશે.',
  yesMarkSafe: '✅ હા, હું સુરક્ષિત છું',
  markedSafe: '✅ તમારી રાઈડ સુરક્ષિત ચિહ્નિત કરાઈ છે. SOS અલર્ટ રદ થયો છે.',
  markSafeFailed: (err: string) => `સુરક્ષિત ચિહ્નિત કરી શકાયું નથી: ${err}`,

  // Errors
  somethingWentWrong: 'કંઈક ખોટું થયું. ફરીથી શરૂ કરવા "book" મોકલો.',
  sessionExpired: 'સત્ર સમાપ્ત થયું. ફરીથી પ્રમાણિત થવા "book" મોકલો.',
  error: (msg: string) => `ભૂલ: ${msg}\nફરીથી શરૂ કરવા "cancel" મોકલો.`,
  rideBeingBooked: 'તમારી રાઈડ બુક થઈ રહી છે. કૃપા કરીને રાહ જુઓ અથવા રદ કરવા "cancel" મોકલો.',

  // Transit ticketing (metro / bus / subway)
  transitTicketButton: '🎫 મેટ્રો/બસ ટિકિટ',
  chooseTransitMode: '🎫 તમે શું બુક કરવા માંગો છો?',
  transitMetro: '🚇 મેટ્રો',
  transitBus: '🚌 બસ',
  transitSubway: '🚊 ઉપનગરીય ટ્રેન',
  transitUnavailable: 'તમારા શહેરમાં હજુ ટિકિટ બુકિંગ ઉપલબ્ધ નથી.',
  enterBoardingStation: (mode: string) => `તમે કયા ${mode} સ્ટેશનથી ચઢશો?`,
  enterDropStation: (mode: string) => `તમે કયા ${mode} સ્ટેશન પર ઉતરશો?`,
  selectBoardingStation: 'તમારું ચઢવાનું સ્ટેશન પસંદ કરો:',
  selectDropStation: 'તમારું ગંતવ્ય સ્ટેશન પસંદ કરો:',
  noStationsFound: (query: string) => `"${query}" માટે કોઈ સ્ટેશન મળ્યું નથી. બીજું નામ અજમાવો:`,
  sameStationChosen: 'ચઢવાનું અને ઉતરવાનું સ્ટેશન એક જ ન હોઈ શકે. બીજું પસંદ કરો:',
  chooseQuantity: 'કેટલી ટિકિટ જોઈએ?',
  ticketsCount: (n: number) => `${n} ટિકિટ`,
  invalidQuantity: (max: number) => `કૃપા કરીને 1 થી ${max} વચ્ચે ટિકિટ પસંદ કરો.`,
  searchingTickets: '🔎 ભાડું તપાસી રહ્યા છીએ...',
  noTicketsAvailable: 'આ રૂટ માટે અત્યારે કોઈ ટિકિટ ઉપલબ્ધ નથી. ફરી શરૂ કરવા "cancel" મોકલો.',
  ticketSummary: (from: string, to: string, fare: number, qty: number) =>
    `🎫 *${from}* → *${to}*\n\n💰 ₹${fare} · ${qty} ટિકિટ`,
  payNow: (fare: number) => `💳 ₹${fare} ચૂકવો`,
  payPrompt: 'ચૂકવણી માટે નીચે ટૅપ કરો. ચૂકવણી થતાં જ હું તમારી ટિકિટ અહીં જ મોકલીશ.',
  awaitingPayment: '⏳ તમારી ચૂકવણીની રાહ જોઈ રહ્યો છું...',
  ticketConfirmed: '✅ *ટિકિટ કન્ફર્મ થઈ ગઈ!*',
  ticketLabel: (ticketNo: string) => `🎫 ટિકિટ ${ticketNo}`,
  ticketValidTill: (till: string) => `⏰ ${till} સુધી માન્ય`,
  ticketQrCaption: (from: string, to: string) => `🎫 ${from} → ${to}\nગેટ પર આ QR બતાવો.`,
  ticketQrFallback: 'QR છબી બનાવી શકાઈ નથી. ગેટ પર આ કોડ બતાવો:',
  paymentFailed: '❌ ચૂકવણી નિષ્ફળ ગઈ. કોઈ રકમ કપાઈ નથી. ફરી પ્રયાસ કરવા "book" મોકલો.',
  paymentTimedOut: '⏰ સમયસર ચૂકવણીની પુષ્ટિ મળી નથી. જો પૈસા કપાયા હોય, તો તમારી ટિકિટ "મારી ટિકિટો" માં દેખાશે.',
  myTicketsButton: '🎫 મારી ટિકિટો',
  noActiveTickets: 'તમારી પાસે કોઈ સક્રિય ટિકિટ નથી.',
  cancelTicket: '❌ ટિકિટ રદ કરો',
  ticketCancelled: '✅ ટિકિટ રદ થઈ. રિફંડ તમારી મૂળ ચૂકવણી પદ્ધતિમાં જમા થશે.',
  ticketCannotCancel: 'આ ટિકિટ હવે રદ કરી શકાતી નથી.',

  // Multimodal journeys
  journeyOptionsHeader: '🧭 ત્યાં પહોંચવાના કેટલાક રસ્તા:',
  transitOptionsLabel: '🚇 જાહેર પરિવહન વિકલ્પો',
  noJourneysFound: 'આ પ્રવાસ માટે કોઈ જાહેર પરિવહન માર્ગ મળ્યો નથી.',
  bookJourney: '🎟️ આ પ્રવાસ બુક કરો',
  bookingJourney: '🎟️ તમારો પ્રવાસ બુક થઈ રહ્યો છે...',
  journeyConfirmed: '✅ *પ્રવાસ બુક થઈ ગયો!*',
  journeyLegsHeader: 'તમારો માર્ગ:',
  journeyCancelled: '✅ પ્રવાસ રદ થયો.',
  cancelJourney: '❌ પ્રવાસ રદ કરો',
  modeWalk: '🚶 ચાલવું',
  modeMetro: '🚇 મેટ્રો',
  modeBus: '🚌 બસ',
  modeTaxi: '🚕 ટૅક્સી',
  modeSubway: '🚊 ટ્રેન',
};
